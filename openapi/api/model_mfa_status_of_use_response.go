/*
SORACOM API

SORACOM API v1

API version: VERSION_PLACEHOLDER
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// MFAStatusOfUseResponse struct for MFAStatusOfUseResponse
type MFAStatusOfUseResponse struct {
	Status *string `json:"status,omitempty"`
}

// NewMFAStatusOfUseResponse instantiates a new MFAStatusOfUseResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMFAStatusOfUseResponse() *MFAStatusOfUseResponse {
	this := MFAStatusOfUseResponse{}
	return &this
}

// NewMFAStatusOfUseResponseWithDefaults instantiates a new MFAStatusOfUseResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMFAStatusOfUseResponseWithDefaults() *MFAStatusOfUseResponse {
	this := MFAStatusOfUseResponse{}
	return &this
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *MFAStatusOfUseResponse) GetStatus() string {
	if o == nil || o.Status == nil {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MFAStatusOfUseResponse) GetStatusOk() (*string, bool) {
	if o == nil || o.Status == nil {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *MFAStatusOfUseResponse) HasStatus() bool {
	if o != nil && o.Status != nil {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *MFAStatusOfUseResponse) SetStatus(v string) {
	o.Status = &v
}

func (o MFAStatusOfUseResponse) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Status != nil {
		toSerialize["status"] = o.Status
	}
	return json.Marshal(toSerialize)
}

type NullableMFAStatusOfUseResponse struct {
	value *MFAStatusOfUseResponse
	isSet bool
}

func (v NullableMFAStatusOfUseResponse) Get() *MFAStatusOfUseResponse {
	return v.value
}

func (v *NullableMFAStatusOfUseResponse) Set(val *MFAStatusOfUseResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableMFAStatusOfUseResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableMFAStatusOfUseResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMFAStatusOfUseResponse(val *MFAStatusOfUseResponse) *NullableMFAStatusOfUseResponse {
	return &NullableMFAStatusOfUseResponse{value: val, isSet: true}
}

func (v NullableMFAStatusOfUseResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMFAStatusOfUseResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


